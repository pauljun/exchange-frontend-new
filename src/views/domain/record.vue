<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      >
        <el-option v-for="(type, k) in types" :key="k" :label="type" :value="k">
        </el-option>
      </el-select>
      <el-input
        v-model="queryParam.rr"
        placeholder="主机记录"
        clearable
        class="filter-item form-search-input"
      />
      <el-select
        v-model="queryParam.line"
        clearable
        placeholder="请选择线路"
        class="filter-item"
        style="margin-right: 5px"
    ...
    >
      ...
      <el-table-column prop="line" label="解析路线">
        <template slot-scope="record"> {{ routes[record.row.line] }} </template
        >value
      </el-table-column>
      ...
      <el-table-column label="操作">
        <template slot-scope="job">
          ...
          <el-button
            type="danger"
            icon="el-icon-delete"
            @click="handleDelete(job.row.recordid)"
          />
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-form :ref="formName" :model="formFieldsData" :rules="rules">
        ...
        <el-form-item label="TTL" :label-width="formLabelWidth">
          <el-select v-model="formFieldsData.ttl" style="width: 90%">
            <el-option v-for="(t, k) in ttls" :key="k" :label="t" :value="k">
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    disOrEnableRecord(record) {
      this.$http
        .put("record/domain/" + record.recordid + "/" + record.status)
        .then((response) => {
