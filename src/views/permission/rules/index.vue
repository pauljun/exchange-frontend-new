  ...
  methods: {
    ...
    getTableFrom() {
      this.$http.get("table/permissions/permission").then((response) => {
        ...
        this.formCreate.rule[1]["control"][0]["rule"][5].props.options =
          this.getComponents();
        this.formCreate.rule[1]["control"][0]["rule"][3].on = {
          focus: this.selectIcon,
        };
      });
    },
    beforeSubmit(row) {
      ...
      if (row.form.component instanceof Array) {
        row.form.component =
          row.form.component.length > 0 ? row.form.component.pop() : "";
      }
      return row;
    },
    renderAfter(form) {
      if (form !== null) {
        this.formCreate.fApi.setValue({
          permission_mark:
            form.permission_mark.indexOf("@") === -1
              ? form.permission_mark
              : form.permission_mark.split("@")[1],
    ...
    afterHandleResponse() {
      ...
      this.$http
        .get("table/permissions/permission", { params: { only: "form" } })
        .then((response) => {
          this.formCreate.rule = response.data.form;
          this.formCreate.rule[1]["control"][0]["rule"][5].props.options =
            this.getComponents();
