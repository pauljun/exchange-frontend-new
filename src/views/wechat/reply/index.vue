  ...
  <div class="app-container">
      ...
      <el-table-column label="操作">
        <template slot-scope="rule">
          ...
          <el-button
            ...
            size="small"
            icon="el-icon-delete"
            @click="handleDelete(rule.row.id)"
            >删除</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="formVisible">
      <el-form :ref="formName" :rules="rules" :model="formFieldsData">
        ...
        <el-form-item
          v-if="formFieldsData.rule_type == 1"
          label="关键字"
          :label-width="formLabelWidth"
          prop="keyword"
        >
          <el-input
            v-model="formFieldsData.keyword"
            placeholder="请输入关键字"
            autocomplete="off"
            clearable
          />
        ...
        <el-form-item
          v-if="formFieldsData.type === '4' || formFieldsData.type === '6'"
          label="上传音频"
          :label-width="formLabelWidth"
          prop="voice_url"
        >
          <el-input
            v-model="formFieldsData.voice_url"
            placeholder="请输入音频链接"
            autocomplete="off"
            clearable
          />
        ...
        <el-form-item
          v-if="formFieldsData.type === '5'"
          label="上传视频"
          :label-width="formLabelWidth"
          prop="video_url"
        >
          <el-input
            v-model="formFieldsData.video_url"
            placeholder="请输入视频链接"
            autocomplete="off"
            clearable
          />
        ...
        <el-form-item
          v-if="
            formFieldsData.type === '1' ||
            formFieldsData.type === '4' ||
            formFieldsData.type === '5' ||
            formFieldsData.type === '7'
          "
          label="内容"
          :label-width="formLabelWidth"
          prop="content"
        ...
        <el-form-item
          v-if="formFieldsData.type === '3' || formFieldsData.type === '4'"
          label="上传图片"
          :label-width="formLabelWidth"
          prop="image_url"
        >
          ...
          >
            <img
              v-if="formFieldsData.image_url"
              :src="formFieldsData.image_url"
              class="avatar"
            />
...
export default {
  ...
  data() {
    return {
      ...
      imageAction:
        process.env.VUE_APP_BASE_API + "/wechat/official/upload/image",
      ...
      rules: {
        ...
        voice_url: [{ required: true, message: "请上传音频" }],
  ...
  methods: {
    ...
    disOrEnable(rule) {
      this.$http
        .put("wechat/official/reply/enable/" + rule.id)
        .then((response) => {
    ...
    beforeVoiceUpload(file) {
      const isMp3 =
        file.type.indexOf("mp4") !== -1 && file.type.indexOf("amr") !== -1;
      if (!isMp3) {
